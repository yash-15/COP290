...
public class Network {
	...
	boolean expose_server,is_server,well_connected;
	int num_users=1,max_priority=1;
	int secretKey=0;
	int goodClient=0;
	String serverAddress,port;//Stores the IP address and port of the server
	
	NetworkBox networkBox;
	ChatBox chatBox;
	
	public Network(Main _parent) {
		parent = _parent;
		
		users=new NetworkPlayer[4];
		sockets=new Vector<Socket>();
		
		networkBox = new NetworkBox(this);
		chatBox=new ChatBox();
	...
	public void setup(){
		...
		try{
			...
			parent.window.setTitle("Initializing....");
		}
		catch(Exception e){
		...
		if (!expose_server){
			try{
				...
				new cl_thread(new ConnectionPackage(c_socket,
		...
		///If you are the starting server add your data to the users array 
			...
			parent.window.setTitle("Pong Game -- Listening @ "+me.ip+":"+me.s_port);
		...
		if (expose_server)
			chatBox.display.append("Server Started by "+users[0].name+"\n");				
		...
		try{
			while(true){   
				...
				new sr_thread(new ConnectionPackage(s_socket1,
	...
	void disconnect(int i) {
		users[i].priority=-1;
		chatBox.status.setText("Number of Users: "+(--num_users));
	...
	public  void connectAll() throws IOException {
		...
		if (well_connected) {
			chatBox.display.append("Successfully connected to the peer-to-peer network\n");
			parent.window.setTitle("Pong Game @IP: "+me.ip);
		}
		goodClient=0;
		int[] toConnect={i1,i2};
		for(int i=0;i<2;i++) {
			...
			if (j>=0) {
				  ...
				  sockets.add(c_socket);
				  users[j].conn=new ConnectionPackage(c_socket,
						  new BufferedReader(new InputStreamReader(c_socket.getInputStream())),
	...
	void newServer() {
		...
		chatBox.display.append("New Server is "+users[min].name+" @ "+serverAddress
				+":"+port+"\n");
	}
	 ...
	 * 
	 */
	String giveName(String t_name,int i) {
		if (t_name.equals("")||t_name.equals(null))
			t_name="NO_NAME";
		t_name=t_name.trim();
	 ...
	 */
	 String socketAddress2localAddress(String sockAddr,char sep) {
		int index=0;
		while(sockAddr.charAt(index)!=sep)
			index++;
		return sockAddr.substring(0,index);
	}
	
	
	private class cl_thread extends Thread{
		...
		public cl_thread(ConnectionPackage conn){
		...
		public void run() {
			try{
				...
				chatBox.display.append("Connected to Server @ "+addr+"\n");
				...
				while(true) {	
					...
					if (t_prot.equals("SUBMITKEYNDATA")){
						chatBox.display.append("Asked for Secret Key and User Info @ "+addr+"\n");
						...
						chatBox.display.append("Sent Secret Key and User Info @ "+addr+"\n");
					}
					else if (t_prot.equals("CONNECTED")){
						chatBox.display.append("Successfully connected to "+addr+"\n");
						...
						chatBox.display.append("Asked for Name and Port by Server\n");
						msgJsonObjectTo=new JSONObject();
						...
						to.println(msgJsonObjectTo.toString());
						chatBox.display.append("Sent name and port to server\n");
					}
					else if (t_prot.equals("USERDATA")){
						chatBox.display.append("Receiving User Data from Server\n");
						 ...
						 for(int j=0;j<4;j++){
							 ...
							 if (users[j].ip.equals(serverAddress)&&
									 users[j].s_port==Integer.valueOf(port)) users[j].conn=new ConnectionPackage(socket,from,to);
						 }
						 ...
						 chatBox.display.append("Received User Data from Server\n");
						 chatBox.display.append("I am "+me.name+"\n");
				...
				while(true){
					...
					if(x.equals(null))
						...
						chatBox.display.append(x+"\n");
				}
				
			}
			catch(Exception e) {
				chatBox.display.append("Server @ "+socket.getRemoteSocketAddress()
						.toString()+"is unavailable.\n");
				
				///If the connection is not established properly that means you must restart
				if (!well_connected){
					JOptionPane.showMessageDialog(null,"The server is unavailable");
					restart();
				}
				///Otherwise you assume that the server is down and continue with that
					...
					for(int j=0;j<4;j++){
						...
						if (users[j].ip.equals(t_ip)
								&& users[j].s_port==Integer.valueOf(t_port)){
							disconnect(j);
							if (t_ip.equals(serverAddress) && t_port.equals(port))
								newServer(); ///Update for new server
							testprint();
							break;
						}
					}
	...
	private  class sr_thread extends Thread{
		...
		public sr_thread(ConnectionPackage conn) {
			this.socket=conn.socket;
			this.from=conn.bfReader;
			this.to=conn.ptWriter;
		}
		...
		public void run() {
			...
			try{
				...
				if (expose_server){
					connect=true;
					chatBox.display.append("Connected to "+remoteAddress+"\n");
					chatBox.status.setText("Number of Users: "+(++num_users));
					expose_server=is_server &&num_users<4;
					...
					users[i].conn=new ConnectionPackage(socket,from,to);
					///attempt to receive name and port in the next exchange
					msgJsonObjectTo.put("PROTOCOL", "SUBMITNAMEPORT");
					to.println(msgJsonObjectTo.toString());
					chatBox.display.append("Asked "+remoteAddress+" for name and port\n");
					...
					if (msgJsonObjectFrom.optString("PROTOCOL").equals("NAMEPORT")) {
						...
						chatBox.display.append(remoteAddress+" --> "+users[i].name+"\n");
						chatBox.display.append("Sending UserData to "+users[i].name+"\n");
						...
						chatBox.display.append("Sent UserData to "+users[i].name+"\n");
						testprint();
					}
					else {
						///Disconnects if any other info is sent
						chatBox.display.append("Incorrect Info! Connection cut @ "+remoteAddress+"\n");
						socket.close();
					}
				}
				 ...
				 */
					 ...
					 */
					connect=false;
					chatBox.display.append("Attempt to connect from "+remoteAddress+"\n");
					if (num_users<4){
						msgJsonObjectTo=new JSONObject();
						msgJsonObjectTo.put("PROTOCOL", "SUBMITKEYNDATA");
						to.println(msgJsonObjectTo.toString());
						chatBox.display.append("Asking for secret Key @ "+remoteAddress+"\n");
						
						///Wait for the response
						msgJsonObjectFrom=new JSONObject(from.readLine());
						chatBox.display.append("Verifying Key @ "+remoteAddress+"\n");
						try{
						...
						if (connect){
							...
							t_user.conn=new ConnectionPackage(socket,from,to);
					 ...
					 */
					if (!connect){
						chatBox.display.append("Connection Refused @ "+remoteAddress+"\n");
				...
				try{
					...
					if(connect){
						disconnect(i);
						chatBox.display.append("Disconnected @ "+remoteAddress+"\n");
					}
				}
			}
		}
