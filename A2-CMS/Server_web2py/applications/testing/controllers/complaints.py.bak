    ...
    if (l<2):
        ...
        if (l==2):
            if (comp_type=="admin_ind"):
                ...
                #Shows all the grp_level complaints which were associated with you 
            else:
                raise HTTP(404)
...
def discard():
    ...
    return dict(complaint=comp,log=db(db.complaint_logs.id==log).select().first())
...
def forward():
    ...
    if (l!=2):
        ...
        child=int(request.args[0])
        cid=int(request.args[1])
        #Admin Authentication
        admin=db((db.administrators.id==child)&(db.administrators.cor_user==auth.user.id)&(db.administrators.parent!=None)).select()
        if (len(admin)==0):
            raise HTTP(404) 
        else:
            admin=admin.first()
            parent=admin.parent
            comp=db((db.grp_complaint.id==cid)&(db.grp_complaint.Cur_Admin_ID==child)&((db.grp_complaint.Status==2)|(db.grp_complaint.Status==3)
                                                              |(db.grp_complaint.Status==6))).select()
            db((db.grp_complaint.id==cid)&(db.grp_complaint.Cur_Admin_ID==child)&((db.grp_complaint.Status==2)|(db.grp_complaint.Status==3)
                                                              |(db.grp_complaint.Status==6))).update(Cur_Admin_ID=parent)
            if (len(comp)==0):
                raise HTTP(404)
            else:
                comp=comp.first()
                log=db.complaint_logs.insert(Complaint_ID=cid,Admin1=child,Admin2=parent,action_taken="Forward",mod_date=datetime.now)
    return dict(complaint=comp,log=db(db.complaint_logs.id==log).select().first())
...
def approve():
    ...
    if(l!=3):
        ...
        if (len(comp)>0):
            ...
            raise HTTP(404)
    return dict(complaint=comp,admin_ind=db(db.ind_complaint.id==admin_ind).select().first(),log=db(db.complaint_logs.id==log).select().first())
