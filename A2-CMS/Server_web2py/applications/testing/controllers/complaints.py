...
def forward():
    ...
    if (l!=2):
        ...
        if (len(admin)==0):
            ...
            admin=admin.first()
            parent=admin.parent
            comp=db((db.grp_complaint.id==cid)&(db.grp_complaint.Cur_Admin_ID==child)).select()
            db((db.grp_complaint.id==cid)&(db.grp_complaint.Cur_Admin_ID==child)).update(Cur_Admin_ID=parent)   
            if (len(comp)==0):
                raise HTTP(404)
            else:
                comp=comp.first()
